...
const findBlockedTimeSlots = async ({db, date, employeeId}) => {
  ...
  const [appointmentResults] = await db.query(savedAppointmentsQuery, [date, employeeId]);
...
router.get(`/`, async (req, res) => {  
  try {
    ...
    const [serviceRows] = await req.dbPool.query(serviceQuery, [serviceId]);
    ...
    `;
    const [employeeAvailabilityRows] = await req.dbPool.query(employeeAvailabilityQuery);
